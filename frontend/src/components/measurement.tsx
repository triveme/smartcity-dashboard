...
export function MeasurementComponent(props: UtilizationProps) {
  ...
  return (
    <Box display='flex' flexDirection='column' height='100%' width='100%'>
      ...
      {/* Handler-Box */}
      <Box display='flex' flexDirection='row' flexBasis='85%' height='100%' width='100%'>
        {/* Chart-Holder-Box */}
          ...
          <Box flexBasis={dayActive ? '90%' : '100%'} height='100%' width='100%'>
            {dayActive ? (
              <Box display='flex' flexDirection='row' justifyContent='center' alignItems='flex-start'>
                <Typography color={colors.iconColor} sx={{ fontSize: '120px' }}>
                  18.29
                </Typography>
                <Typography color={colors.grey} sx={{ fontSize: '40px' }}>
                  cm
                </Typography>
              </Box>
            ) : null}
