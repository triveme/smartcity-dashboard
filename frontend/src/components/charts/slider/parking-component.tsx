...
export function ParkingComponent(props: ParkingComponentProps) {
  ...
  const handleShowOnMapClick = () => {
    if (selectedIndex >= 0 && sliders[selectedIndex].location) {
      showOnMap(
        selectedIndex,
        sliders[selectedIndex].location.latitude,  
        sliders[selectedIndex].location.longitude,
      );
    }
  }
        ...
        >
          ...
          <Box>
            {sliders.map((obj, index) => {
              return (
                <Box
                  key={"SliderWrapper-Box-" + obj.name}
                  display={"flex"}
                  flexDirection={"row"}
                >
                  <SliderWithKnobs name={obj.name} currentValue={obj.currentlyUsed} maximumValue={obj.maxValue}></SliderWithKnobs>
                  <Box
                    paddingLeft={"5px"}
                    paddingBottom={"6px"}
                    margin="auto"
                  >
                    <IconButton
                      onClick={() => handleParkingspaceClickOpen(obj, index)}
                    >
                      <DashboardIcon
