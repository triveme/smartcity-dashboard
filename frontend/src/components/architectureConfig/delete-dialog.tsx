...
export function DeleteDialog(props: DeleteDialogProps) {
  ...
  const handleDelete = () => {
    ...
    if (type === 'Dashboard') {
      newCurrentArchitectureContext = newCurrentArchitectureContext.filter((d: DashboardComponent) =>
        component.uid ? d.uid !== component.uid : d._id !== component._id,
      )
    } else if (type === 'Widget') {
      const dashboardIndex = newCurrentArchitectureContext.findIndex((d: DashboardComponent) => d.name === parents[0])
      if (dashboardIndex !== -1)
        newCurrentArchitectureContext[dashboardIndex].widgets = newCurrentArchitectureContext[
          dashboardIndex
        ].widgets.filter((w: WidgetComponent) => (component.uid ? w.uid !== component.uid : w._id !== component._id))
    } else if (type === 'Panel') {
      const dashboardIndex = newCurrentArchitectureContext.findIndex((d: DashboardComponent) => d.name === parents[0])
      const widgetIndex = newCurrentArchitectureContext[dashboardIndex].widgets.findIndex(
        (w: WidgetComponent) => w.name === parents[1],
      )
      if (dashboardIndex !== -1 && widgetIndex !== -1)
        newCurrentArchitectureContext[dashboardIndex].widgets[widgetIndex].panels = newCurrentArchitectureContext[
          dashboardIndex
        ].widgets[widgetIndex].panels.filter((p: PanelComponent) =>
          component.uid ? p.uid !== component.uid : p._id !== component._id,
        )
    }
  ...
  return (
    <div>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id='delete-dialog-description' sx={{ color: colors.textDark }}>
