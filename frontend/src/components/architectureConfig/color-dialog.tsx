...
export function ColorDialog(props: ColorDialogProps) {
  ...
  const updatedColorArray = (attrColor: string) => {
    let colArray: string[] = currentTab.apexOptions!.colors
      ? currentTab.apexOptions!.colors
      : [];
    if (
      currentTab.apexOptions!.colors &&
      currentTab.attribute &&
      currentTab.attribute.keys.length > colArray.length
    ) {
      // missing colors, fill missing values
      for (
        let i = 0;
        i < currentTab.attribute.keys.length - colArray.length;
        i++
      ) {
      ...
      <DialogActions style={{ justifyContent: "center" }}>
        <SaveButton
          ...
          customStyle={{
            fontWeight: "bold",
            color: "white",
            backgroundColor: !currentTab.apexMaxColor
              ? colors.primary
              : currentTab.apexMaxColor,
          }}
        />
      </DialogActions>
    </Dialog>
