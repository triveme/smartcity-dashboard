...
export function ArchitectureEditButtons(
  ...
  const handleClickDeleteClose = () => { setDeleteOpen(false); };

  // New panel
  const [panelCreationOpen, setPanelCreationOpen] = useState(false);
  const handlePanelDialogOpen = () => { setPanelCreationOpen(true); };
  ...
  const handleDashboardDialogOpen = () => { setDashboardCreationOpen(true); };
  const handleDashboardCreationClose = () => { setDashboardCreationOpen(false); };

  // New widget
  const [widgetCreationOpen, setWidgetCreationOpen] = useState(false);
  const handleWidgetDialogOpen = () => { setWidgetCreationOpen(true); };
  const handleWidgetCreationClose = () => { setWidgetCreationOpen(false); };
    ...
    if (type === "Dashboard") {
      ...
      if (numOfDashboards > 1) {
        const dashboardIndex = architectureContext.currentArchitectureContext.findIndex(
            (d: DashboardComponent) => d.name === component.name && (component._id ? (d._id === component._id) : (d.uid === component.uid))
          );
      ...
      if (dashboardIndex !== -1) {
        const widgetIndex = architectureContext.currentArchitectureContext[
          dashboardIndex
        ].widgets.findIndex((w: WidgetComponent) => (w.uid ? w.uid : w._id) === parentsUids[1]);
        if (widgetIndex !== -1) {
          const numOfPanels =
