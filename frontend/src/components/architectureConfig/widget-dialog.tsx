...
export function WidgetDialog(props: WidgetDialogProps) {
  ...
  const [newWidgetLinks, setNewWidgetLinks] = useState<any[]>(widget.infoLinks && widget.infoLinks[0] ? widget.infoLinks : [{ infoLinkTitle: '', infoLinkUrl: 'https://', infoLinkDescription: '' }]);
  ...
  const editWidget = () => {
    if (newWidgetName.length > 0) {
      let newArchitectureContext = cloneDeep(architectureContext);
      let newCurrentArchitectureContext = cloneDeep(newArchitectureContext.currentArchitectureContext);
      let dashboardIndex = newCurrentArchitectureContext.findIndex((d: DashboardComponent) => (d._id !== "" ? d._id : d.uid) === parentDashboardUid);
      let widgetIndex = newCurrentArchitectureContext[dashboardIndex]
        .widgets.findIndex((w: WidgetComponent) => (widget.uid ? (w.uid === widget.uid) : (w._id === widget._id)));
        ...
        <Box display="flex" alignItems="center">
          <IconButton key={"selected-widgetTabIcons-IconButton"} onClick={handleTabIconsChooserClickOpen}>
            {newWidgetTabIcons.map((icon) => (
              <WidgetTabIcons
                key={"selected-widget-tabIcons-" + icon}
                icon={icon} 
                color={colors.iconColor}
              />
            ))}
              ...
              {iconList.map((icon) => (
                ...
                >
                  <DashboardIcon
                    ...
                    icon={icon}
                    color={
                      newWidgetIcon === icon ? colors.iconColor : colors.white
                    }
                  />
                </IconButton>
              ))}
          ...
          <DialogContent>
            <Box component="form" sx={{ display: "flex", flexWrap: "wrap" }}>
              {widgetTabIconList.map((icon) => (
                <IconButton
                  key={"box-tabIcons-" + icon}
                  onClick={() => handleIconClickInTabIcons(icon)}
                >
                  <WidgetTabIcons
                    key={"dashboardicon-tabIcons-" + icon}
                    icon={icon}
                    color={
                      newWidgetTabIcons.indexOf(icon) > -1 ? colors.iconColor : colors.white
                    }
                  />
                </IconButton>
              ))}
