    ...
    >
      ...
      <Box display='flex' alignItems='center'>
        <SmallField
          ...
          onChange={(e) => setNewPanelValue('height', parseInt(e.target.value) ? parseInt(e.target.value) : 300)}
          onBlur={() => {
      ...
      <Box display='flex' alignItems='center' paddingBottom={1}>
        <FormControl variant='outlined' sx={{ minWidth: 150 }} style={{ marginTop: 9 }}>
          <InputLabel id='demo-simple-select-filled-label'>Tabs (Anzahl / aktuell)</InputLabel>
          <Select
            size='small'
            margin='dense'
          ...
          >
            ...
            >
              <RadioGroup
                ...
                value={currentTabIndex < tempPanel.tabs.length ? currentTabIndex : tempPanel.tabs.length - 1}
      ...
      <TabConfigurator currentTabIndex={currentTabIndex} tempPanel={tempPanel} setNewPanelValue={setNewPanelValue} />
    </Paper>
  )
}
