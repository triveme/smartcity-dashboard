...
export class QueryService {
  ...
  constructor(
    ...
    private readonly sqlViewService: SqlViewDataService,
  ...
  async enqueueQueryPopulation(
    ...
    if (queryBatches.length === 0) {
      throw new NotFoundException(
        `No SQL View query found with id: ${queryId}`,
      );
    }
  ...
  async getQueuedQueryData(
    ...
    return this.sqlViewService.executeQueuedFetch({
      ...
      fingerprintInput: {
        ...
        unorderedCollectionPaths: ['queryConfig.attributes'],
      },
      execute: (signal) =>
        this.sqlViewService.getDataFromDataSource(effectiveBatch, signal),
    });
  }
  ...
  private async enqueueQueryData(
    ...
    await this.sqlViewService.executeQueuedFetch({
      category,
      priority: 'background',
      fingerprintInput: {
        platform: 'sql-view',
        ...
        target: {
          ...
          view: queryBatch.query_config.fiwareService,
        },
      ...
      execute: async (signal) => {
        const newData = await this.sqlViewService.getDataFromDataSource(
          queryBatch,
          signal,
        );
        signal.throwIfAborted();
        const dataToSave =
          this.transformationService.convertAttributesToFiware(newData);
        signal.throwIfAborted();
  ...
  private getEffectiveQueryConfig(queryBatch: QueryBatch): object {
    return {
      ...
      view: queryBatch.query_config.fiwareService,
      attributes: queryBatch.query_config.attributes,
    };
  }
  ...
  async getAllQueriesWithAllInfos(): Promise<QueryWithAllInfos[]> {
    return this.db
      ...
      .where(eq(dataSources.origin, 'sql'));
  }
  ...
  private async getQueryWithAllInfos(
    ...
    return this.db
      ...
      .where(and(eq(dataSources.origin, 'sql'), eq(queries.id, queryId)));
  }
