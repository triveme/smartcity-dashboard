...
export class PanelDataService {
  ...
  async downloadPanelData(panelId: string): Promise<string> {
    try {
      ...
      const panelToWidgets =
        await this.widgetsToPanelsRepo.getByPanelId(panelId);
      ...
      // Array to hold CSV data for all widgets
      ...
      console.error(
        'Error downloading data for panel with id:',
        panelId,
        '\ndue to error: ',
        error,
      );
