...
export class InternalQueryPopulationController {
  constructor(private readonly usiPlatformService: UsiPlaformService) {}
  ...
  async populateQuery(
    ...
    await this.usiPlatformService.enqueueQueryPopulation(
      queryId,
      request.roles ?? [],
      request.tenant,
    );
  ...
  async getQueryData(
    ...
    return this.usiPlatformService.getQueuedQueryData(
      queryId,
      overrides,
      request.roles ?? [],
      request.tenant,
