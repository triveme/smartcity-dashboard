...
export class InternalQueryPopulationController {
  constructor(private readonly orchideoService: OrchideoConnectService) {}
  ...
  async populateQuery(
    ...
    await this.orchideoService.enqueueQueryPopulation(
      queryId,
      request.roles ?? [],
      request.tenant,
    );
  ...
  async getQueryData(
    ...
    return this.orchideoService.getQueuedQueryData(
      queryId,
      overrides,
      request.roles ?? [],
      request.tenant,
